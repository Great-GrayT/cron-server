/**
 * Central configuration. All secrets/URLs come from the environment — nothing
 * sensitive is hardcoded. Ported from the original app; RSS_STATS_FEED_URLS
 * added for the stats ingestion pipeline.
 */

export const RSS_FEED_URLS = process.env.RSS_FEED_URLS
  ? process.env.RSS_FEED_URLS.split(",").map((url) => url.trim()).filter(Boolean)
  : [
...
export const CHECK_INTERVAL_MINUTES = parseInt(process.env.CHECK_INTERVAL_MINUTES || "60", 10);
...
// Stats ingestion feeds (separate from the notification monitor)
export const RSS_STATS_FEED_URLS = process.env.RSS_STATS_FEED_URLS
  ? process.env.RSS_STATS_FEED_URLS.split(",").map((url) => url.trim()).filter(Boolean)
  : [];

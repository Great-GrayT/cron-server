...
// parser below does not expand entities, but we reject such bodies outright as
// defense-in-depth before any further processing.
const DTD_PATTERN = /<!DOCTYPE|<!ENTITY/i;
...
async function parseSingleFeed(url: string): Promise<JobItem[]> {
  try {
    ...
    // SSRF guard: resolve + validate the feed host before opening any socket.
    ...
    if (response.status === 304 && cached) {
      logger.info(`[RSS] 304 cached (${cached.jobs.length} jobs): ${url.substring(0, 60)}...`);
    ...
    if (DTD_PATTERN.test(xmlText)) {
      throw new RSSParseError("Feed contains DOCTYPE/ENTITY declarations (XXE guard)", url);
    }
    logger.info(`[RSS] 200 (${xmlText.length}b): ${url.substring(0, 60)}...`);
